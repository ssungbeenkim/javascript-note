...
// flow는 왼쪽에서 오른쪽으로 함수를 수행합니다. 문자열에서 시작해 headalize(), emphasize(),
// imagify(), linkify(), 마지막으로 codify()를 수행한 뒤 문자열을 반환한다.

// flow()는 확실히 더 깔끔하고 이해하기도 좋다.

// 이 함수를 한번만 사용해야 한다면? 즉시 실행 시킬 수 있다.
{
...
/* 
pipe() 함수는 두 가지 중요한 지점에서 flow()와 다르다.
...
const comments3 = pipe(
  ...
  join('\n'),
  chaoticListify
); // chaoticListify() 함수를 추가했다. 이런 식으로 파이프라인에 함수를 계속해서 추가할 수 있다.
